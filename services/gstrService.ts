...
// Types for GSTR operations
export interface GSTR1Return {
    id: string;
    tenantId: string;
    returnPeriod: string; // YYYYMM
...
export class GSTRService {
     ...
     */
    static async generateGSTR1FromInvoices(tenantId: string, returnPeriod: string): Promise<void> {
        try {
            ...
            const { data: invoices, error } = await supabase
                ...
                .select(`
          *,
          customers!inner(name, gstin, state_code),
          invoice_items(product_name, hsn, sac, qty, rate, unit, taxable_value, gst_percent)
        `)
                .eq('tenant_id', tenantId)
     ...
     */
    private static async processInvoiceForGSTR1(gstr1ReturnId: string, invoice: any): Promise<void> {
     ...
     */
    private static async addToB2CS(
        gstr1ReturnId: string,
     ...
     */
    private static async addToNilSupplies(
        gstr1ReturnId: string,
        amount: number,
        description: string
    ): Promise<void> {
     ...
     */
    private static async updateGSTR1Totals(gstr1ReturnId: string): Promise<void> {
     ...
     */
    static async generateGSTR3BFromGSTR1(tenantId: string, returnPeriod: string): Promise<void> {
        try {
     ...
     */
    private static async populateSection3_1_a(gstr3bReturnId: string, gstr1ReturnId: string): Promise<void> {
        ...
        const aggregated: { [key: string]: { taxable: number, igst: number, cgst: number, sgst: number } } = {};
     ...
     */
    private static async populateSection3_2(gstr3bReturnId: string, gstr1ReturnId: string): Promise<void> {
        ...
        // Aggregate by place of supply
        const aggregated: { [key: string]: { taxable: number, igst: number } } = {};
     ...
     */
    private static async populateSection3_1_c(gstr3bReturnId: string, gstr1ReturnId: string): Promise<void> {
        const { data: nilData } = await supabase
            .from('gstr1_nil')
            .select('description, nil_amount, exempt_amount, non_gst_amount')
            .eq('gstr1_return_id', gstr1ReturnId);

        (nilData || []).forEach(async (item) => {
            await supabase
                .from('gstr3b_3_1_c')
                .insert({
                    gstr3b_return_id: gstr3bReturnId,
                    description: item.description,
                    nil_amount: item.nil_amount,
                    exempt_amount: item.exempt_amount,
                    non_gst_amount: item.non_gst_amount
                });
        });
    }
     ...
     */
    private static getFinancialYear(returnPeriod: string): string {
     ...
     */
    static async getGSTRReturns(tenantId: string): Promise<{
        gstr1: GSTR1Return[],
        gstr3b: GSTR3BReturn[]
    }> {
